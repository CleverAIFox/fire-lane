/* Fire-Lane · 3단 마스크
   ────────────────────────────────────────────────────────────
   동명동                원본 밝기      — 판정 대상
   display_scope − 동명동  살짝 어둡게  — mask_soft (fill-opacity .42)
   그 밖                 덮는다         — mask      (fill-opacity .9)

   안전센터는 동명동 밖에 있다(대인 서 1.0km / 지산 동 1.2km).
   동 경계만 잠그면 출동 경로가 화면에서 잘린다.

   이 파일은 모양을 만들지 않는다. 두 폴리곤은 publish_web.py 가 굽는다 —
     mask.geojson       = 세계 − display_scope
     mask_soft.geojson  = display_scope − 동명동
   display_scope 는 src/firelane/seg/scope.py 의 규칙이다 —
     동 경계 50m ∪ 안전센터 반경 300m ∪ 회랑 버퍼 70m ∪ 동 경계 여백 60m
     를 합친 뒤 폭 300m 미만의 틈과 안쪽 구멍을 메운 것(DECISIONS §182-7).
   ★ 2026-09-22 (W9-5) 정정. 가운데 층을 「접근 회랑」이라고만 적고 있었다.
     실제 mask_soft 는 회랑만이 아니라 **안전센터 반경 원 · 동 경계 여백 ·
     메운 틈까지** 전부다. 어둡게 덮인 곳을 전부 출동 경로로 읽으면 틀린다.
