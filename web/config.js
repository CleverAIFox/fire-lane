...
const CONFIG = {
  ...
  /* 마커. 원기둥 몇 개를 쌓아 실제 시설의 실루엣을 근사한다.
   ...
   ★ 소화전만 덜 줄인 이유 — 11개뿐이라 CCTV 104·가로등 폴 천여 개 사이에
  ...
  markers: [
    ...
    /* ★ 축소 이력. 16.8 → 10.9(x0.65, 2026-08) → 6.54(x0.6, 2026-09-01).
       ...
       ★ 다른 마커가 x0.4 인데 여기만 x0.6 이다. 11개뿐이라 같이 줄이면
         CCTV 104개·가로등 폴 천여 개 사이에서 사라진다. */
  ...
  /* ★ 2026-09-20 (PLAN §1 #62). 발행 구간은 세 층이다 —
       대상지  동명동 안                       출동 대상 그 자체
       회랑    동 밖 + 출동경로가 쓰는 구간     여기로 들어온다
       참고    동 밖 + 어떤 경로도 안 쓰는 구간  자료 수집 범위에 들어왔을 뿐
     지금까지 셋이 **같은 진하기**로 그려져 「어디가 대상지인가」가 안 보였다.
     ★ 색은 안 쓴다. 색은 판정 4종의 것이고, 범례에 없는 색이 지도에만
       남는 상태가 제일 나쁘다(layers/segments.js). 진하기로만 가른다. */
  scope: {
    corridorAlpha: 0.62,   // 회랑 구간 투명도
    asideAlpha   : 0.28,   // 참고 구간 투명도. 배경이지 주인공이 아니다
  },
  ...
  /* ── 패널 레이어 토글 — 이름·줌·해상도의 **유일한 선언** ─────────
     ★ 2026-09-22 (PLAN §13 W9-4). 토글 이름이 index.html 에 문자열로 박혀
       있었다 — `항공영상 25cm (줌 15↑)`. `줌 15↑` 는 map.js 의 `minzoom:15`
       와 잇는 코드가 없었고, `25cm` 는 sources.yaml 을 사람이 베낀 것이었다.
       하나를 고치면 다른 하나가 조용히 거짓이 된다.
     ★ 이제 화면 문구와 지도 동작이 **같은 값**을 읽는다.
         label  토글에 뜨는 이름
         zoom   이 줌부터 그린다. 레이어가 실제로 이 값을 minzoom 으로 쓴다
                (ortho → map.js · poles → layers/poles.js)
         labelZoom  이 줌부터 글자가 뜬다(poi → layers/poi.js 의 상호 라벨)
         res    해상도 표기. 사본이다 — 정본은 sources.yaml ortho 의
                「지상표본거리 25cm」이고 tests/test_web_labels.py 가 대조한다
       문구는 js/ui/toggles.js 의 layerLabel() 이 조립한다.
       예: 항공영상 (25cm · 줌15+)
     ★ 마커 토글 이름은 여기가 아니라 markers[].label 이다(같은 원리). */
  layers: {
    dispatch : { label:"🚒 출동 모드" },
    buildings: { label:"건물 3D" },
    poles    : { label:"가로등 폴", zoom:16 },    // 천여 점(개수는 lightpoles.geojson)이라 그 아래는 소음이 된다
    ortho    : { label:"항공영상", res:"25cm", zoom:15 },
    poi      : { label:"상가", labelZoom:17 },    // 점은 줌16부터, 상호는 labelZoom 부터
    terrain  : { label:"지형 굴곡" },
    mask     : { label:"스코프 밖 가리기" },
    theme    : { label:"라이트 모드" },
  },
  ...
  /* ── 가로등 폴 ───────────────────────────────────────────
     ★ 2026-08-23 배선. 그전까지 publish_web.py 가 lightpoles.geojson
       천여 점을 발행하는데 web/js 어디에서도 읽지 않았다. 브라우저가
       요청조차 안 했다(고아 레이어).

     ★ 가로등이 두 개인 이유.
         streetlights   46지점 · 573등  지번 대표점이라 ±50m 오차.
                        등 수와 관리번호는 이쪽이 정본이다
         lightpoles  천여 점          수치지형도 C0220000. 실제 폴 위치.
                        구분(보안등/가로등/조명등)만 있고 등 수는 없다
       둘은 다른 데이터다. 위치를 보려면 이쪽, 등 수를 보려면 저쪽이다.

     ★ 마커(3D extrusion)로 만들지 않는다. 1,188개를 폴리곤으로 세우면
       무겁고, 무엇보다 소화전·CCTV 같은 "시설" 과 위계가 같아져 버린다.
       가로등은 배경 정보라 점으로 깐다. poi 와 같은 취급이다. */
  poles: {
    /* 그리기 시작 줌은 layers.poles.zoom 이다(토글 이름과 같은 값). */
    /* ★ 2026-09-20 (W9-1). 키가 `"보안등"` 이었다. 실물은 `"보안(방범)등"` 이고
       그것이 **664점 — 전체의 55.9%** 다. `poles.js` 의 `match` 는 정확 일치라
       그 664점이 전부 `other` 회색으로 떨어졌고, **파랑은 화면에 한 점도 없었다.**
       아무것도 안 울었다 — 색이 틀린 것은 예외가 아니라 fallback 으로 흡수된다.
       그것이 이 저장소가 부르는 1족(무음 통과)이고, `match` 의 fallback 이 그 자리다.
       `tests/test_map_colour_keys.py` 가 이제 **발행물 값 집합과 이 표를 대조**한다 —
       키를 고치는 것은 인스턴스고 그 검사가 족이다. */
    color: { "보안(방범)등": "#7fd4ff", "가로등": "#ffd98a", "조명등": "#c9b6ff",
             other: "#9aa4b2" },
    haloColor: "#141a26",
  },

  poi: {
    /* 상호가 뜨는 줌은 layers.poi.labelZoom 이다(토글 이름과 같은 값). */
