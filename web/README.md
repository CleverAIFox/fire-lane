...
| 파일 | 내용 |
|---|---|
| `index.html` | 뼈대. 패널 마크업. 토글 **이름은 비어 있고** `config.js` `CONFIG.layers` 에서 채운다 |
| `style.css` | 색·간격·타이포·레이아웃 |
| `config.js` | 색상표·임계값·마커 형상·카메라·**레이어 선언**(이름·줌·해상도) |
| `js/main.js` | 부트스트랩. 초기화 순서만 |
| `js/data.js` | ★ 데이터 접근 단일 지점 |
| `js/layers/` | 레이어·판정 렌더링 |
| `js/icons/` | 표지판 캔버스 그림 |
| `js/ui/` | 범례·검색·테마·토글·미니맵 |
| `data/` | 생성물. `publish_web.py` 산출. 손으로 고치지 말 것 |

소유(리뷰)의 정본은 `.github/CODEOWNERS` 다 — 여기에 사본을 두지 않는다(`MASTER §8` ·
2026-09-22 PLAN §13 W3-2 닫힘). 종전 이 표의 「주인」 칸은 CODEOWNERS 와 갈려 있었다.
