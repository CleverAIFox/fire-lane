...
def test_build_tags_sources_and_compare_flags_suspects():
    ...
    assert t.loc["B", "edge_src"] == "fallback" and t.loc["B", "suspect"] == ""
...
def test_far_offset_is_flagged_before_width_gap():
    ...
    row = {"share": 0.9, "offset_m": 6.2, "edge_width": 13.9, "width_min_m": 0.97, "bldg_ngii_m": 2.7, "edge_bldg_ngii_m": 0.0}
    t = S.suspect(row)
    assert "멀리" in t and "폭불일치" in t and "건물관통" in t
    assert S.priority(t) > S.priority("폭불일치") > S.priority("")
    assert "멀리" not in S.suspect({**row, "offset_m": 0.4})
