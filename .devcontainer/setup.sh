...
#   판은 `web/navi/.nvmrc` 를 따른다(devcontainer.json features · MASTER §17-1 node). 클래식 JS 검사(`node --check` ·
...
#   판의 정본은 Dockerfile 이고 따르는 자리는 MASTER §17-1 표와
#   tests/test_sources_of_truth.py 의 SPEC 이 든다.
