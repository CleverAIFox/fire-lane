...
CRITICAL = (
    ...
    # ── 외부 대조 ────────────────────────────────────────────
    # ★ 2026-09-23 (DECISIONS §222-6). `feeds` 를 산문에서 리스트로 바꾸자마자
    #   대장과 이 표가 **한 건 어긋난 것**이 드러났다. `seg/report.py` 가 읽는데
    #   관문 밖이었다 — 산문일 때는 기계가 못 읽어서 아무도 몰랐다.
    #   MASTER §4 는 소방서 지정 구간 대조를 **우리 폭에 대한 유일한 외부 대조
    #   수단**이라고 든다. 그것이 조용히 비면 판정이 틀려도 견줄 것이 없어진다.
    "fire_access",        # 소방통로(소방서 지정 구간). seg/report.py 의 nfa_compare — 외부 대조축
)

# 계보상 통과로 보는 상태. SKIP 은 "이번 실행에서 건드리지 않음"이다.
PASS_STATUS = ("OK", "SKIP")

# ★ 2026-09-23 (DECISIONS §224-3). 단계가 **메모리로** 죽었을 때의 종료코드.
#   정본은 여기 하나다 — `ingest` 가 내고 `pipeline` 이 읽는다. 양쪽에 숫자를
#   박으면 그것이 곧 갈린다(MASTER §18-3).
#   왜 가르나: 종전에는 ingest 가 어떻게 죽었든 1 이었고, 파이프라인이 그것을
#   「소스가 실패했다」 로 읽어 `--retry-failed` 를 찍었다. 그 명령은 "실패한
#   소스가 없다" 를 내놓는다 — **안내가 거짓이었다.**
...
# ★ 2026-09-23 (DECISIONS §224-3). 메모리로 죽은 것을 **메모리로 죽었다고**
#   말한다. 종전에는 `OSError: [Errno 12]` 트레이스백만 나가고 종료코드가 1
#   이어서, 파이프라인이 그것을 「소스가 실패했다」 로 읽고
#   `--retry-failed` 를 찍었다. 그 명령은 "실패한 소스가 없다" 를 내놓는다 —
#   **안내가 거짓이었고 사람이 그 말을 따라 헛돌았다.** 이 저장소가 가장
#   싫어하는 1족이다(MASTER §17).
#
#   구분은 종료코드로 한다. 출력을 파싱하면 문구를 다듬는 순간 검사가 죽는다
#   (`pages_add_navi` 앵커와 같은 자리). 값의 정본은 `guards.ENOMEM_RC` 다.


def run_stage(main) -> None:
    """단계 모듈의 `main()` 을 돌리되, **메모리로 죽은 것을 메모리로 죽었다고** 말한다."""
...
def oom_exit(what: str) -> None:
    ...
    print("  그 뒤:  uv run fire-lane --from ingest", file=sys.stderr)
