...
**② 내륜차.** 회전할 때 뒷바퀴가 앞바퀴보다 안쪽으로 돈다.
   그만큼 폭이 더 필요하다.
       ...
       내륜차 ≈ L² / (2R)          L = 축거 · R = 회전반경

   L=4.0m · R=8.0m 이면 1.0m 다. 직선에서 3.0m 로 통과하던 길이
   급커브에서는 4.0m 를 요구한다.
...
def offtracking(radius_m: float) -> float:
    ...
    ★ 정확히는 `R - √(R² - L²)` 이고 위 식은 그 1차 근사다.
      R 이 L 에 가까워지면(초저속 급선회) 근사가 무너지므로 그때는
      정확식을 쓴다. 골목에서 R < 8m 인 곳이 실제로 있다.
    """
...
def edge_cost(length_m: float,
    ...
    ── lenient ─────────────────────────────────────────────────
    `unknown` 352구간(32%)과 `needs_cv` 190구간은 **모른다** 는 뜻이지
