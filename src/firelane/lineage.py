...
lineage.py — 무엇으로 만들어졌나. 단계별 입출력 지문의 정본.
    ...
    python -m firelane.lineage show      기록된 계보를 본다
    python -m firelane.lineage verify    현재 디스크와 대조한다
...
def verify(processed: Path, root: Path, step, expand, steps,
    ...
    fresh — **이번 실행에서 방금 성공한 단계 이름들.**

    ★ 2026-08-22. 세 번째 계보 교착. 전량 실행(`fire-lane`)이 ingest 를
      돌려 _manifest.json 을 새로 쓴 직후, segments 의 ③ 자가대조가
      "내가 지난 실행에 읽은 것과 다르다" 며 막았다.

          segments 이 지난 실행에 읽은 것  sha 36d3d7a7
          지금 디스크                    sha c77dea55

      당연히 다르다 — 방금 상류가 다시 썼으니까. **상류가 이번 실행에서
      돈 것과 안 돈 것을 구분하지 않은 것이 결함이다.**

      08-18 사고(ngii1k 가 14336 으로 기록됐는데 segments 는 옛 레이어
      6675 개를 읽음)는 **상류가 이번 실행에 안 돈** 경우다. 그 방어는
      그대로 살아 있어야 하고, 지금 고치는 것은 그 반대 경우다.

      ②(상류 대조)도 같은 이유로 건너뛴다. 상류가 방금 쓴 것을 상류가
      방금 쓴 기록과 비교하는 것은 항상 참이라 검사가 아니다.
    """
    fresh = fresh or set()
    lg = load(processed)
    if not lg:
    ...
    # 이번 실행에서 방금 갱신된 산출물. 대조 대상이 아니다.
    _fresh_keys = {_key(root, q) for s in steps if s.name in fresh
                   for q in expand(s.produces)}
    ...
    for p in expand(step.consumes):
        ...
        #   달라야 정상이다. 여기서 막으면 전량 실행이 항상 깨진다.
        if k in _fresh_keys:
            continue
            ...
            if was is not None:
                if not _same(was, now):
                    problems.append(
                        ...
                        f"      {up} 가 쓴 것  {was}\n"
                        f"      지금 디스크    {now}")
                # ★ 2026-08-22. 여기서 반드시 끝낸다.
                #   종전에는 ② 가 **통과해도** ③ 으로 흘러갔다. ② 통과는
                #   "지금 디스크는 상류가 쓴 바로 그것" 이라는 뜻이고, 그것이
                #   입력 검증의 전부다. 그런데 ③ 이 이어서 "그런데 내가 지난
                #   실행에 읽은 것과는 다르다" 며 막았다.
                #
                #   그것은 오류가 아니라 **다시 돌아야 할 이유**다. 상류가
                #   갱신됐으면 하류는 당연히 새 입력으로 돌아야 한다.
                #   이 결함 때문에 전량 실행이 ingest 를 갱신할 때마다
                #   segments 에서 깨졌다(2026-08-22, 222초를 버렸다).
                #
                #   ③ 은 상류 기록이 없는 입력(raw 등)을 위한 것이다.
                #   상류가 보증한 입력까지 자가대조할 이유가 없다.
                continue
