...
probe.py — 데이터를 파이프라인에 넣기 전 통과시키는 진단기.
    ...
    python -m firelane.probe crs  data/raw/xxx.shp [--kr]
        .prj를 무시하고 좌표값만으로 실제 좌표계를 역추정한다.
        "좌표계 통일했는데 어긋난다"의 원인을 30초 만에 특정한다.

    python -m firelane.probe topo data/processed/road_link.geojson
...
from firelane.krgis.crs import CRS_METRIC, GWANGJU_BBOX, KOREA_BBOX, probe_crs
