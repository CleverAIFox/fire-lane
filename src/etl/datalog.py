...
datalog.py — 데이터 대장 도구
    ...
    uv run python src/etl/datalog.py record      매니페스트 갱신(파이프라인 끝에서 호출)
    uv run python src/etl/datalog.py graph       의존 그래프 → docs/lineage.mmd
    uv run python src/etl/datalog.py impact KEY  이 소스를 바꾸면 깨지는 것
    uv run python src/etl/datalog.py backup DIR  외장으로 복사 + sha 기록
    uv run python src/etl/datalog.py verify DIR  외장 대조
    uv run python src/etl/datalog.py check       대장 정합성 검사
