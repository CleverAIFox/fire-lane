...
inventory.py — 원본에 실제로 무엇이 들어 있는지 훑어 대장에 박는다.
    ...
    uv run python src/etl/inventory.py            훑고 sources.yaml 갱신
    uv run python src/etl/inventory.py --dry      갱신 없이 출력만
    uv run python src/etl/inventory.py --only ngii1k
...
from paths import RAW, ROOT  # noqa: E402
import ngi  # noqa: E402
