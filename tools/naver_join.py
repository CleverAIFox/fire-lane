...
def main() -> int:
    ...
    # ── 판정 ───────────────────────────────────────────────────
    # ★ 2026-08-23. 표본이 작으면 산포 추정 자체가 흔들린다. 그것을 말하지
    #   않고 "쓸 수 있다" 고 단언하면, 10개 재고 전수로 넘어갔다가 나중에
    #   뒤집힌다. 이 저장소가 소방서 7구간을 게이트로 쓰고 그 결과를
    #   "외부 검증" 이라 부른 것과 같은 종류의 실수다.
    #
    #   표준편차의 95% 상한을 같이 낸다(정규근사, se = 1/√(2(n-1))).
    #     n=10 · sd 0.20  →  참값이 0.32 까지 갈 수 있다
    #     n=10 · sd 0.35  →  참값이 0.56 까지 — 기준 0.5 를 넘는다
    import math
    n = len(w)
    sd_hi = sd * math.exp(1.96 / math.sqrt(2 * (n - 1))) if n > 1 else float("inf")

    print("\n── 판단")
    if n < 20:
        print(f"   ※ 표본 {n}개는 **잠정**이다. 산포 {sd:.2f} 의 95% 상한이"
              f" {sd_hi:.2f} 다.")
        if sd < 0.5 <= sd_hi:
            print("     상한이 기준(0.5)을 넘는다 — 지금 결론을 확정하지 마라.")
            print("     20개까지 채우면 구간이 좁아진다.")
        else:
            print("     상한도 기준 안이면 그때 결론이 선다.")
