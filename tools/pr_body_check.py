...
def _section(body: str, title: str) -> str:
    ...
    ★ 2026-08-31. `---` 를 절 끝으로 보지 않았다. `계약을 건드리는가` 가
      템플릿의 마지막 `##` 절이라 그 아래 `<details>` 안의 체크리스트까지
      같은 절로 셌고, **템플릿을 성실히 채우면 `[x]` 가 다섯이 되어 운다.**
      안내문을 지우고 한 줄 쓴 사람은 통과했다 — 강제자가 반대로 작동했다.
      나흘간 안 걸린 이유는 검사가 옳아서가 아니라 아무도 접힌 블록을
      채우지 않았기 때문이다. 역방향은 `tests/test_pr_body_check.py` 가 든다.
    """
