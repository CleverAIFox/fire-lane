...
def rules() -> list[tuple[str, str, str]]:
    ...
    out = [
        ...
        # ★ 2026-09-24 (PLAN §12 #20 닫힘 · DECISIONS §237). 파급 다섯은 위에서
        #   2026-09-02 에 걷혔는데 **수를 만든 표 자체가 남았다.** 제목이
        #   「목록」이라 전수를 주장하고, 실제로는 대장 72종 중 12행 발췌다.
        #   같은 판단을 제목에도 적용한다 — **수를 안 적는다.** 적으면 대장이
        #   늘 때마다 또 낡고, 그것이 이 행이 석 주를 산 이유다.
        (r"□ 원천 데이터 목록",
         "□ 주요 원천 데이터 — 전수는 sources.yaml 대장이 정본이다",
         "표는 발췌인데 제목이 전수를 주장했다"),
    ...
    # ── 판정 모델 (MASTER §2-2 · §3-4) ──
    #   ★ 2026-09-24 (PLAN §13 W12-3). `PLAN §12-4` 를 들고 있었다 — **그런 절은 없다.**  ref-ok
    #     §12 에는 하위 절이 하나도 없고 있던 적도 없다. 근거는 MASTER 쪽 둘이다.
    ...
    # 공개율 논거는 소방용수시설 전체가 분모여야 맞다(MASTER §3-12).
    #   ★ 2026-09-24 (W12-3). `§16-3` 이라고 적혀 있었다 — §16 의 하위는 16-1 · 16-2 둘뿐이다.
    out += [
    ...
    # ── 과장 · 미검증 (MASTER §15-2 「말하지 않는 것」) ──
    # ★ 2026-09-24 (DECISIONS §244). 리스크 표만 「해소」를 선언하고 있었다.
    #   같은 문서 두 곳(§3 서술 · §5 제원)은 「추정으로 남는다」고 맞게 적는다 —
    #   **심사에서 가장 아플 종류의 어긋남**이라 리스크 표 쪽을 사실로 맞춘다.
    out += [
        # ★ `blocked` 191 은 **하한**이다. MASTER §15-2 가 「확정 개수로 쓰지
        #   않는다」를 하지 않을 말로 명시하는데, 외부가 읽는 이 문서만 단서가
        #   빠져 있었다(README 는 달고 있다).
        # ★ 두 자리에 있다(§1 요약 · §4 수치 표기 원칙). 길이를 아끼려고
        #   근거는 MASTER 로 위임한다 — 긴 칸 래칫이 이 표를 보고 있다.
        (r"통행\s*불가\s*191(?!\s*\(하한)", "통행 불가 191(하한 · MASTER §2)",
         "MASTER §2 · §15-2 — 확정 개수로 쓰지 않는다"),
        # ★ CCTV 유효범위 25m 는 미확인 값이다. 기획서는 「조사 결과에 따라」로
        #   검증된 것처럼 적었다.
        (r"폴백\s*버퍼\s*반경은\s*실제\s*CCTV\s*식별\s*가능\s*거리\s*조사\s*결과에\s*따라\s*25m로\s*한다\.",
         "폴백 버퍼 반경 25m 는 **미확인 잠정값**이다. 이 값 하나가 243구간의 "
         "판정을 가르므로 현장 답사로 검증한 뒤 확정한다.", "MASTER §7-2 · §16-1"),
    ]

    # ── 이미 확보한 것을 「미확보」로 적는 자리 (MASTER §3-12 · §6) ──
    # ★ 소화전 좌표는 2026-08-31 에 확보됐고 `web/data/hydrants.geojson` 157건이
    #   배포 중이다. 「좌표 확보 이전까지 지도에서 제외한다」는 조건이 끝났다.
    out += [
        (r"소화전\s*레이어는\s*좌표\s*확보\s*이전까지\s*지도에서\s*제외한다\.\s*"
         r"1개짜리\s*레이어는\s*만들지\s*않느니만\s*못하다\.",
         "소화전 좌표는 확보됐다 — 스코프 안 157지점을 표출한다. "
         "관할 589 대 표준데이터 동구 445 의 분모 차이는 남는다.",
         "web/data/hydrants.geojson · MASTER §3-12"),
    ]

    # ── 배포 방식 (시제) ──
    # ★ EC2 · FastAPI · 상태 DB 는 **후속 설계**인데 현재형으로 적혀 지금 그렇게
    #   도는 것으로 읽힌다. 실제 배포는 GitHub Pages 정적 배포다.
    out += [
        (r"FastAPI,\s*상태\s*DB,\s*프론트엔드를\s*Docker\s*Compose로\s*일원화하여\s*"
         r"EC2\s*단일\s*인스턴스에\s*배포한다\.",
         "현재 배포는 GitHub Pages 정적 배포이며 web/data 를 계약으로 읽는다. "
         "FastAPI · 상태 DB · EC2 는 실시간 판정 연동 시점의 후속 설계다.",
         ".github/workflows/deploy.yml"),
    ]

    # ── 일정 (PLAN §1 #4 가 정본) ──
    out += [
        (r"MVP\s*2026-09-30", "MVP 통합 2026-10-08", "PLAN #4"),
    ]

    # ── 폭 소스 모델 (MASTER §3-5) ──
    # ★ 2026-09-24 (PLAN §12 · DECISIONS §243). 기획서가 **1:5,000 도로경계면을
    #   주 소스**라고 적는다. 실물의 주 소스는 1:1,000 수치지형도 도로경계이고
    #   1,281 중 1,166(91%)이 그것이다 — 1:5,000 의 채택은 **1건**이다
    #   (`data/golden/segments.fingerprint.json` L1.width_src).
    #   숫자가 아니라 **모델**이 어긋난 자리라 `docx_check` 가 구조적으로 못 잡는다.
    # ★ 레이어명도 2026-08-17 에 `NF_A_A01000` → `N3A_A0010000` 으로 바뀌었다.
    #   개명 규칙을 **먼저** 둔다 — 아래 규칙들이 새 이름으로 쓰여 있다.
    out += [
        (r"NF_A_A01000", "N3A_A0010000", "2026-08-17 레이어 개명"),
        (r"도로폭은\s*수치지도\s*도로경계면\(N3A_A0010000,\s*1:5,000\)을\s*주\s*소스로,\s*"
         r"실폭도로\(TL_SPRD_RW\)를\s*폴백으로\s*사용한다\.",
         # ★ 길이를 늘리지 않는다 — 이 칸은 `docx_check` ⑧ 의 긴 칸 래칫 안이다.
         "도로폭은 1:1,000 도로경계가 주 소스이고(1,166/1,281), "
         "1:5,000 도로경계면(N3A_A0010000)·실폭도로(TL_SPRD_RW) 순으로 폴백한다(1·112).",
         "MASTER §3-5"),
        (r"국토지리정보원\s*수치지도\s*2\.0\s*—\s*도로경계면\s*N3A_A0010000\s*\(1:5,000\)",
         "국토지리정보원 1:1,000 수치지형도 도로경계(주 소스) · "
         "수치지도 2.0 도로경계면 N3A_A0010000(1:5,000 · 폴백)", "MASTER §3-5"),
        (r"수치지도\s*미커버\s*세그먼트는\s*실폭도로로",
         "1:1,000 · 1:5,000 미커버 세그먼트는 실폭도로로", "MASTER §3-5"),
    ]

    # ── 배경지도 · 배포 (2026-09-22 · MASTER §12) ──
    # ★ 2026-09-24. 둘 다 **이미 안 쓰는 것**을 현재형으로 적고 있었다.
    #   V-World 는 옛 지도와 함께 걷혔고(DECISIONS §218-6 · `web/config.js` 에서
    #   블록을 지웠다) 배경은 자체 정사영상 타일 1,423장이다. 배포는 SSH 가
    #   아니라 SSM Run Command 다 — **키를 Secrets 에 넣지 않는다**(MASTER §12).
    #   외부가 읽는 문서가 「키를 서버에 둔다」로 읽히면 그것 자체가 사고다.
    out += [
        (r"MapLibre\s*\+\s*V-World\s*기반", "MapLibre + 자체 정사영상 타일 기반", "§218-6"),
        (r"MapLibre\s*GL\s*JS\s*\+\s*V-World\s*배경지도",
         "MapLibre GL JS + 자체 정사영상 타일(z15~19)", "§218-6"),
        (r"V-World\s*배경지도,\s*4색", "정사영상 배경 타일, 4색", "§218-6"),
        (r"\[구현 주의\]\s*V-World\s*WMTS[^\n]*",
         "[구현 주의] 배경은 자체 정사영상 타일이다(web/data/ortho/{z}/{x}/{y}.jpg · "
         "z15~19 · 1,423장). 외부 WMTS 키를 쓰지 않으므로 키 신청·도메인 등록·"
         "축 순서 함정이 모두 없어진다.", "§218-6"),
        (r"지도\s*스택\s*MapLibre\s*\+\s*V-World\s*확정",
         "지도 스택 MapLibre 확정", "§218-6"),
        (r"EC2\s*1대\s*\+\s*Docker\s*Compose\s*\+\s*SSH",
         "EC2 1대 + Docker Compose + SSM Run Command", "MASTER §12 — 키를 Secrets 에 안 넣는다"),
    ]
    ...
    out += [
        ...
        # ★ 2026-09-24 (DECISIONS §244). 여기 옛 규칙 둘이 있었다. 그 규칙이
        #   바꿔 넣은 결과가 **「축거·회전반경은 동부소방서 인터뷰로 확정」**이고,
        #   **그 확정은 오지 않았다** — D-30 질의에서 전폭·전장만 답이 왔고
        #   축거·최소회전반경은 공식 규격에 없다(`wheelbase_verified: false`).
        #   규칙이 오지 않은 미래를 과거형으로 박아 두고 있었다. 원문이 이미
        #   사라졌으므로 옛 규칙은 걷고, **그 규칙이 남긴 문장**을 찾는다.
        (r"축거·회전반경은\s*동부소방서\s*인터뷰로\s*확정\.",
         "축거·최소회전반경은 공식 규격에 없어 추정값으로 남는다"
         "(wheelbase_verified: false).", "MASTER §3-13 · PLAN #20"),
        (r"축거·회전반경\s*확보\s*시\s*내륜차를\s*재산출한다\.",
         "축거·최소회전반경을 확보하면 내륜차를 재산출한다.", "MASTER §3-13"),
    ]
...
def inserts() -> list[tuple[str, str, str]]:
    ...
    return [
        ...
        ("③ 문제 정의 — 소방차는 구간의 평균 폭이 아니라 가장 좁은 병목을 통과하지 못한다. 그래서 판정에 쓰는 값은 최소 유효폭이고, 관측하지 못한 구간은 통과 가능으로 칠하지 않고 회색으로 남긴다.",
         ...
         "같음"),
        ("④ MVP 범위 — 판정 파이프라인(공개 데이터 · 한 명령으로 재현) · 관제 지도(판정 4색 · 여유폭 색과 수치 · 빨강의 사유) · 내비(통행 가능 경로 · 턴바이턴 · 일방통행 · 회전 금지). CCTV 실시간 연동과 가상환경은 범위 밖이며 후속 계획으로 둔다.",
         "⑤ POC — 산출단위 1,281구간에 통행 가능 465 · 판정 보류 226 · 통행 불가 191(하한 · MASTER §2) · 영상판정 불가 399 를 냈다. CCTV 유효범위 25m 안에 드는 구간이 451 / 1,281(35.2%)이라 영상판정이 닿지 않는 골목이 남으며, 그 사실을 회색으로 드러낸다. 폭 실측 대조(D-25)는 진행 중이다.",
         "같음"),
