...
### 20-2. 비용 함수는 있으나 배선되지 않았다

`seg/vehicle.py::edge_cost()` 가 폭 · 내륜차 · 회전반경 · 판정을 받아
비용을 낸다. `math.inf` 면 못 간다.
...
★ `lenient` 가 필요한 이유 — `unknown` 과 `needs_cv` 를 합치면 **전체의
절반**이다. 모르는 곳을 전부 막으면 그래프가 끊겨 경로가 아예 안 나온다.

**이 함수는 아직 `access_corridor()` 에 연결되어 있지 않다.**
`tools/route_probe.py` 가 둘을 비교만 하고 아무것도 바꾸지 않는다.

### 20-3. 배선하면 무엇이 바뀌는가

`route_probe` 가 거리만 쓰는 경로와 차량 비용을 쓰는 경로를 비교한다.
2026-09-01 기준으로 **거리만 쓰면 지나가는 통행 불가 구간이 세 자리수**다.
회전 불가 구간도 따로 나온다 — 폭과 무관하게 최소회전반경이 모자란 곳이다.

★ **이 차이가 이 절의 존재 이유다.** 지금 지도가 잘못 안내하는 구간이며,
배선하는 순간 그 사실이 사라진다. 그래서 여기 적는다. 현재 숫자는
`uv run python tools/route_probe.py` 가 준다.

배선은 `access_corridor()` 의 `weight="length"` 를 `edge_cost()` 로 바꾸는
것이다. 바꾸면 `route_usage` 가 움직이고 `golden` 이 운다. **재잠금이
필요하다**(§14-4).

### 20-4. 탐색 알고리즘은 미확정이다

데이터 흐름도는 `A*` 로 적혀 있으나 **구현은 Dijkstra 다.** 대상이 동명동
한정이고 노드 규모가 작아 휴리스틱의 이득이 크지 않다. 확정 전이며
바꾸더라도 비용 함수는 그대로 쓴다.

★ 흐름도는 **설계**이고 이 절은 **실물**이다. 둘이 다르면 실물이 옳다.
