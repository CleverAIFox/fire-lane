...
★ **오창준은 2026-09-07 에 이탈한다.** `@woongtopia/gis` 팀에서 빼면
...
`ruleset_check` 의 `ADMINS` 가 이 명단이며 실물과 다르면 운다. 09-07 에
...
회수일은 2026-09-07 이다. 룰셋 실물과의 대조는 `ruleset_check` 가 한다.


★ **bypass 회수일은 오창준 이탈일(2026-09-07)이며 검사가 든다.** 그날이 지나면
...
`bypass_actors` 때문이었고 **2026-09-07 회수 후에는 막힌다.** 승인은
