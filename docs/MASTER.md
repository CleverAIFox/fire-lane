...
강제자  `tests/test_doc_style.py` · `tools/dms.py`(마지막 줄 「새 규칙을 적을 때는 강제자를 같이 만든다」를 세는 자리가 이 도구다 — **분모 0**(2026-09-25 · 절 1,033 전수. 종전 232 는 그 도구가 절을 틀리게 세던 때의 수다. 물림 312 도 **전부 선언됐고 수까지 적혀 있다** — 하위 절이 늘면 수가 어긋나 운다)) · `tools/scopedecl.py`(강제자가 자기 범위를 선언하는가)
