...
★ **커버리지는 래칫이다.** 2026-09-19 실측 **24%**(16,069 stmts · 12,275 miss).
  `tools/verify.sh` 의 `COV_MIN=23` 으로 걸려 있고 **올린 뒤에는 안 내린다.**
  80% 를 목표로 잡지 않는다 — 못 지키는 문턱은 끄게 되고, 끈 문턱은 없는 것과 같다.
  실측 24 에 23 을 거는 것은 화면의 `24%` 가 반올림이라 실제가 23.5 일 수
  있기 때문이다. 딱 붙이면 다음 실행이 오차로 빨개진다.
