...
대장은 `sources.yaml` 하나다. `datasets` 67종 · `retired` 9종.
...
현재 `retired` 9종이 있다. 전부 파일이 있는 보관본이다 — 파일 없는 기각 기록은 대장에 두지 않고 `landing_disposition` 과 DECISIONS 가 든다.
