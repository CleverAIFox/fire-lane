...
| 색 | verdict | 뜻 |
|---|---|---|
| 초록 | `clear` | 주차가 있어도 통과. 영상판정 불필요 |
| 주황 | `needs_cv` | 도면만으로는 결론이 안 남. 영상판정 대상 |
| 빨강 | `blocked` | 차가 없어도 통과 불가 |
| 회색 | `unknown` | 폭을 산출할 재료가 없음 |

**`needs_cv`(주황)가 이 프로젝트의 미완성 부분이다.** 호모그래피(영상판정)가
아직 안 붙었고, 붙으면 이 268개 구간이 초록이나 빨강으로 갈린다.
나머지 색은 도면만으로 확정된 것이라 바뀌지 않는다.
