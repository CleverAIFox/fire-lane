...
`web/data/stations.geojson` — 소방서·안전센터 3건. `kind` 로 나뉜다.

| `kind` | 뜻 | 표시 |
|---|---|---|
| `center` | 119안전센터. **출동 시작점** | 빨간 원 + 헤일로 |
| `station` | 동부소방서. 관할 본서 | 주황 링 |

동부소방서는 대인119안전센터와 **주소가 같다**(제봉로 210). 좌표가 겹치므로
본서를 바깥 링으로 그려 구분한다. 출동은 소방서가 아니라 안전센터에서 나간다.

`web/data/hydrants.geojson` — 공개된 소화전 11개. **동명동 안은 1개뿐이다.**
동부소방서 관할 588개 중 31개(5%)만 공개된 상태이고, 이 희소성 자체가 발표 논거다.
**버그가 아니다.** 패널에 `1 / 11` 로 표기된 것이 그 뜻이다.

`web/data/cctv.geojson` — 177지점 312대. 원 크기 = `카메라대수`.
가로등·보안등은 공개 데이터가 없어서, 야간 시인성 논거는 CCTV 분포로 대체한다.
